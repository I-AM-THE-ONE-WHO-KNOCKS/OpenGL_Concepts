...
void init(){
    ...
    mProgramID = mShader->init("shader_clipping.vert", "shader_clipping.frag");
    if(mProgramID < 0){
    ...
    mClippingLocation = glGetUniformLocation(mProgramID, "clippingPlane");
    if(mClippingLocation < 0){
       printf("invalid location for clippingPlane\n");
    }
    glUniform4f(mClippingLocation,-1.0,0.0,0.0,0.0);
